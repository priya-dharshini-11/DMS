...
def reset_dms_cycle(user_id, event_type, cur=None):
    if event_type not in ("LOGIN", "ACTIVITY_VERIFIED", "RESTORE"):
    ...
    if event_type == "LOGIN":
        ...
        description = "Account restored after deletion request"
    else:
        description = "User verified activity"
...
def permanently_delete_due_accounts():
    """
    Permanently delete accounts whose 30-day deletion grace period
    has expired.

    DELETION_PENDING accounts remain fully recoverable until their
    ...
    try:
        ...
        for user in users:
            ...
            # Preserve a minimal admin audit record before
            # permanently deleting the user's account data.
            cur.execute("""
                ...
                VALUES
                    (
                        ...
                        NOW(),
                        'Account permanently deleted after the 30-day recovery period.'
                    )
            """, (
                user["id"],
                user["name"],
        ...
        if users:
            print(
                f"Permanent deletion processor removed "
        ...
        if user and check_password_hash(user["password_hash"], password):
            ...
            if user["account_status"] == "DELETION_PENDING":
                ...
                flash(
                    ...
                    "Restore it from the Account page within 30 days."
                )
...
def restore_account():
    ...
    try:
        ...
        if user["account_status"] != "DELETION_PENDING":
            mysql.connection.rollback()
            cur.close()
            flash("Your account is not currently scheduled for deletion.")
            return redirect(url_for("account"))

        # Cancel any unfinished release transaction.
        cur.execute("""
            SELECT id
            FROM releases
            WHERE user_id=%s
        ...
        for release in releases:
            ...
            cur.execute("""
                ...
                SET
                    status='CANCELLED',
                    error_message='Release cancelled because account was restored.'
                WHERE release_id=%s
                  AND status IN ('PENDING', 'FAILED')
            """, (release_id,))
        ...
        cur.execute("""
            INSERT INTO activity_history
                (user_id, event_type, event_time, description)
            VALUES
                (%s, 'ACCOUNT_RESTORED', NOW(),
                 'Account restored before permanent deletion. A fresh DMS cycle was started.')
        """, (user_id,))
...
def request_account_deletion():
    ...
    try:
        # Lock the account row so two deletion requests cannot race.
        cur.execute("""
            SELECT
                id,
                account_status,
        ...
        if user["account_status"] == "DELETION_PENDING":
            mysql.connection.rollback()
            cur.close()
            flash("Your account is already scheduled for deletion.")
            return redirect(url_for("account"))

        if user["account_status"] != "ACTIVE":
            mysql.connection.rollback()
            cur.close()
            flash("Your account cannot be deleted from its current state.")
            return redirect(url_for("account"))
        ...
        # -------------------------------------------------
        # Cancel unfinished releases.
        #
        # Already SENT deliveries are left untouched.
        # Pending/failed deliveries are cancelled.
        # -------------------------------------------------
        ...
        for release in releases:
            ...
            cur.execute("""
                ...
                SET
                    status='CANCELLED',
                    error_message='Release cancelled because account deletion was requested.'
                WHERE release_id=%s
                  AND status IN ('PENDING', 'FAILED')
            """, (release_id,))
            ...
            cur.execute("""
                INSERT INTO activity_history
                    (user_id, event_type, event_time, description)
                VALUES
                    (%s, 'RELEASE_CANCELLED', NOW(),
                     'Release cancelled because account deletion was requested.')
            """, (user_id,))
        ...
        cur.execute("""
            INSERT INTO activity_history
                (user_id, event_type, event_time, description)
            VALUES
                (%s, 'ACCOUNT_DELETION_REQUESTED', NOW(),
                 'Account deletion requested. Account scheduled for permanent deletion after 30 days.')
        """, (user_id,))
    ...
    flash(
        ...
        "You have 30 days to restore it before permanent deletion."
    )

    return redirect(url_for("account"))


def get_current_user_account_status():
    if "user_id" not in session:
        return None
...
def require_active_user():
    ...
    status = get_current_user_account_status()
...
def admin_activity():
    ...
    cur.execute("""
        ...
        SELECT
            ...
            ada.event_time,
            ada.description
        FROM account_deletion_audit ada
